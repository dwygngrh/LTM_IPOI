...
def interp(x, y, xg, yg, data):
    """Bilinear interpolation for 2D grids (Surface or Sliced Layers)."""
...
def interp3d(x, y, z, xg, yg, zg, data):
    ...
    # 1. Check vertical boundaries
    if z <= zg[0]: 
        return interp(x, y, xg, yg, data[0, :, :])
    if z >= zg[-1]: 
        return interp(x, y, xg, yg, data[-1, :, :])
    
    # 2. Find the depth layer index
    k = np.searchsorted(zg, z) - 1
    
    # 3. Interpolate within the two adjacent layers
...
def get_uvw_total(lon, lat, depth, u, v, w, uw, vw, l_c, a_c, z_c, l_w, a_w, is_debris, leeway, dim_mode):
    ...
    dim_mode: 0 = 2.5D, 1 = 3DUV, 2 = 3DUVW
    """
    if dim_mode == 0:
        # 2.5D: Use surface currents (Layer 0) for horizontal movement, no ocean W
        uc = interp(lon, lat, l_c, a_c, u[0, :, :])
        vc = interp(lon, lat, l_c, a_c, v[0, :, :])
        wc = 0.0
    elif dim_mode == 1:
        # 3DUV: Depth-aware horizontal currents, but no ocean W
        uc = interp3d(lon, lat, depth, l_c, a_c, z_c, u)
        vc = interp3d(lon, lat, depth, l_c, a_c, z_c, v)
        wc = 0.0
    else:
        # 3DUVW: Full 3D advection using uo, vo, and wo
        uc = interp3d(lon, lat, depth, l_c, a_c, z_c, u)
        vc = interp3d(lon, lat, depth, l_c, a_c, z_c, v)
    ...
    ut, vt, wt = uc, vc, wc
    # Add wind leeway only for surface-active debris
    if is_debris:
...
def update_3d_safe(lons, lats, depths, active_mask, biofilms, u, v, w, uw, vw, 
    ...
    """Consolidated 2.5D/3DUV/3DUVW Lagrangian kernel supporting multiple schemes."""
    DEG_TO_M = 111320.0 
    abs_dt = np.abs(dt)
    is_backward = dt < 0 
    ...
    for i in prange(len(lons)):
        ...
        if scheme_idx == 0: # Euler Forward
            u1, v1, w1 = get_uvw_total(lons[i], lats[i], depths[i], u, v, w, uw, vw, 
                                       l_c, a_c, z_c, l_w, a_w, is_debris, leeway, dim_mode)
